...
def build_plot(uav_path, obstacles, offset, scale):
    ...
    path_trace = go.Scatter3d(
        x=uav_aligned[:, 0], y=uav_aligned[:, 1], z=uav_aligned[:, 2],
        mode='lines', line=dict(color='blue', width=4), name='UAV Path')

    box_lines = []
    for obs in obstacles:
        name = obs['name']
        if name.startswith("UCX_") and name[4:] in [o['name'] for o in obstacles]:
            continue
        box_lines.extend(draw_box(obs['location'], obs['dimensions'], obs['rotation']))

    fig = go.Figure(data=[path_trace] + box_lines)
    fig.update_layout(scene=dict(xaxis_title='X', yaxis_title='Y', zaxis_title='Z'),
                      title="UAV Path vs Obstacles", showlegend=True)
    return fig
